...
- macOS / Linux
- C言語コンパイラ (clang / gcc)
- GNU Readline ☆要確認：macOSでHomebrew経由のreadlineが必要か
...
- パイプライン構造を正しく表現できるAST設計
	...
	対策として、トークン列を構文木として表現する方針を採用し、再帰下降パーサーによってASTを構築する設計とした。ASTのデータ構造は、コマンドノード（CMD）とパイプノード（PIPE）をunionで統一し、ノードの種類を抽象化する形でC言語上で擬似的なポリモーフィズムを実現した。これにより、パイプラインの入れ子構造をシンプルに表現でき、後続のコマンド実行処理との分離も実現した。

	☆要確認：実装中に詰まった具体的なAST設計のエピソードがあれば1〜2行追加する（例：「リダイレクションをCMDノードの属性とするか、独立ノードにするかで悩んだ」など）
...
- unionを用いたC言語での擬似的なポリモーフィズムの実現
	...
	ASTのノードはCMD（コマンド）とPIPE（パイプ）の2種類があり、それぞれ持つべきデータが異なる。これをunionで統一した構造体として表現し、ノード種別を示すタグと組み合わせることで、C言語でも多態的にノードを扱える構造とした。実行処理側ではノード種別に応じてディスパッチする形式となり、構文木の走査と実行を分離して実装できた。
...
- シグナル処理における状態管理
	...
	対策として、グローバル変数による状態共有とreadlineの制御（`rl_done` など）を組み合わせることで、状態ごとの適切なシグナル処理を実現した。
...
- 低レイヤを知りたい人のためのCコンパイラ作成入門 (Rui Ueyama) (https://www.sigbus.info/compilerbook)
	...
	- ファイル名: src/lexer/, src/parser/ ☆要確認：実際のディレクトリ構成
...
- シェル実装の課題を完走した (JUN's Blog) (https://jun-networks.hatenablog.com/entry/2021/07/02/034002)
	...
	- ファイル名: src/parser/, src/exec/, src/builtin/, src/env/ ☆要確認
...
- The GNU Readline Library を使った (メモの日々) (http://ogawa.s18.xrea.com/tdiary/20080319p02.html)
	...
	- ファイル名: src/signal/, src/main.c ☆要確認